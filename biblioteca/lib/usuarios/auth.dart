...
Future<User?> signUp(
  ...
  try {
    ...
    if (user != null) {
      // 2. Crear el documento en la colección 'usuaris'
      // Si la colección no existe, Firebase la creará automáticamente ahora
      await FirebaseFirestore.instance.collection('usuaris').doc(user.uid).set({
        ...
        'fotoUrl': null,
        'dataRegistre':
            FieldValue.serverTimestamp(), // Recomendado para saber cuándo se creó
      });

      // Actualizamos el nombre en el objeto User de Auth
      await user.updateDisplayName(nom);
    }
    return user;
  } catch (e) {
    // Si hay un error (ej. el correo ya existe), lo lanzamos para mostrarlo en la UI
    throw e.toString();
  }
}
